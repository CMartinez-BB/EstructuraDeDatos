...
public void guardar(ActionEvent event){
    String profesor=this.profesor.getText();
    String materia=this.materia.getText();
    //String comboPrioridad=this.ComboPrioridad.getSelectionModel().getSelectedItem().toString();//
    String Combo=(String) ComboPrioridad.getValue();
    String telefono=this.telefono.getText();
    String dirección=this.dirección.getText();
    String tarea=this.tarea.getText();

    Agenda nota=new Agenda(profesor,materia,ComboPrioridad,telefono,dirección,tarea);
    if(profesor.isEmpty()||materia.isEmpty()||telefono.isEmpty()||dirección.isEmpty()||tarea.isEmpty()){
        Alert alert=new Alert(Alert.AlertType.INFORMATION);
        alert.setTitle("Datos invalidos");
        alert.setTitle("Por favor revice de nuevo los datos ingresados");
    }else{
         this.listaagenda.add(nota);
         this.table.setItems(listaagenda);
    }
